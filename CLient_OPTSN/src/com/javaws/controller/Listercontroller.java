...
public class Listercontroller {
	...
	public Listercontroller(UIListe uiliste,UIAuthentification uIauthentification)
	{
		...
		List<Clients> client;
		//List<ServiceActive> clients = new ArrayList<ServiceActive>();
		try {
			client = service.giveListeClient();
			for(int i = 0;i < client.size();i++)
			{
				System.out.println(client.get(i));
				//clients.add(ServiceActive.APPEL);
				//client.get(i).setListeService(clients);
			}
