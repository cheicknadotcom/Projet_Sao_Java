...
import com.javaws.server.ServeurCreerClientGprsWS;
import com.javaws.server.ServeurCreerClientHlrWS;
import com.javaws.server.ServeurCreerClientInWS;
import com.javaws.server.ServeurDeleteClientGprsWS;
import com.javaws.server.ServeurDeleteClientHlrWS;
import com.javaws.server.ServeurDeleteClientInWS;
...
public class AdministrateurController {
	...
	ServeurCreerClientGprsWS serviceCreerGprs;
	ServeurCreerClientHlrWS serveurCreerHlr;
	ServeurCreerClientInWS serveurCreerIn;
	ServeurDeleteClientHlrWS serveurDeleteHlr;
	ServeurDeleteClientInWS serveurDeleteIn;
	ServeurDeleteClientGprsWS serveurDeleteGprs;
	...
	private void addListener() {
		...
		uiadministrateur.adddeleteInListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				String numero = uiadministrateur.getNumeroField();
				Clients client;
				serveurDeleteIn = new ServeurDeleteClientInWS();
				serveurDeleteGprs = new ServeurDeleteClientGprsWS();
				try {
					client = service.searchByNumTel(numero);
					if(numero.compareTo("")== 0) {
						Utilitaire.displayNotification("Veuillez donner un numero a supprimer...");
					...
					if(client.getType().compareTo("PREPAID") == 0) {
						service.deleteClientIn(numero);
						serveurDeleteIn.deleteClientIN(numero);
					}else if(client.isActivedata())
					{
						service.deleteClientIn(numero);
						serveurDeleteGprs.deleteClientGPRS(numero);
					}else{
