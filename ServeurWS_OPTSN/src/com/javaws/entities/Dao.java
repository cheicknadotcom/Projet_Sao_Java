...
/**********************************************************************************************************************************************************/
	...
	public String searchByNumTelType(String numeroTel) throws Exception
	{
		Clients client = searchByNumTel(numeroTel);
		if(client.getType().compareTo("PREPAID") == 0)
		{
			return TypeClient.PREPAID.name();
		}else if(client.getType().compareTo("POSTPAID") == 0)
		{
			return TypeClient.POSTPAID.name();
		}
		return null;
...
@Override
public Boolean searchByNumTelData(String numeroTel) throws Exception {
	Connection connection = DBManager.getConnection();
    ...
    if (resultSet.next()) {		    	
        Clients client =new Clients(resultSet.getInt("id"), resultSet.getString("date"), resultSet.getString("numeroTel"),TypeClient.valueOf(resultSet.getString("type")),resultSet.getBoolean("data"));
        return client.isActivedata();
    }
    return false;
}
}
